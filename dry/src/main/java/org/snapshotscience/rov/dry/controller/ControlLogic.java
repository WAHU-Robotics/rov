...
public class ControlLogic {
  ...
  public static SetMotion genMotorValues(PolledValues rawPrimaryJoy, PolledValues rawSecondaryJoy) {
    ...
    */
    ...
    vectors.put(ThrustAxis.SURGE, (int) (m.getAxis(Config.SURGE_AXIS) * (Config.SURGE_INVERT ? -1 : 1) * Config.SURGE_SENS));
    vectors.put(ThrustAxis.SWAY, (int) (m.getAxis(Config.SWAY_AXIS) * (Config.SWAY_INVERT ? -1 : 1) * Config.SWAY_SENS));
    vectors.put(ThrustAxis.HEAVE, (int) (m.getAxis(Config.HEAVE_AXIS) * (Config.HEAVE_INVERT ? -1 : 1) * Config.HEAVE_SENS));
    vectors.put(ThrustAxis.YAW, (int) (m.getAxis(Config.YAW_AXIS) * (Config.YAW_INVERT ? -1 : 1) * Config.YAW_SENS));
    vectors.put(ThrustAxis.ROLL, (int) (m.getAxis(Config.ROLL_AXIS) * (Config.ROLL_INVERT ? -1 : 1) * Config.ROLL_SENS));
